...
def parse_args():
    ...
    parser.add_argument(
        "-metric", default="avg_inc_acc", choices=["avg_inc_acc", "last_acc"]
    )
...
def train_func(config, reporter):
    ...
    all_acc = []
    for i, (avg_inc_acc, last_acc, _, is_last) in enumerate(
        inclearn.train.train(train_args)
    ):
        if is_last:
...
def main():
    ...
    if args.tune is not None:
        ...
        tune.run(
            ...
            config=config,
            resources_per_trial={"cpu": 2, "gpu": args.gpu_percent},
            local_dir=args.ray_directory,
            resume=args.resume,
        )

        args.ray_directory = os.path.join(
            args.ray_directory, args.tune.rstrip("/").split("/")[-1]
        )
