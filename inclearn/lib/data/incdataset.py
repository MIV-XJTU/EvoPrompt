...
class IncrementalDataset:
    ...
    def new_task(self, memory=None, memory_val=None):
        ...
        if self._all_test_classes is True:
            ...
            max_class = sum(
                self.increments[: self._current_task + 1 + self._all_test_classes]
            )
            logger.info(
        ...
        if memory is not None:
            ...
            x_train, y_train, train_memory_flags = self._add_memory(
                x_train, y_train, *memory
            )
        else:
            train_memory_flags = np.zeros((x_train.shape[0],))
        if memory_val is not None:
            logger.info(
                "Set validation memory of size: {}.".format(memory_val[0].shape[0])
            )
        ...
        train_loader = self._get_loader(
            x_train, y_train, train_memory_flags, mode="train"
        )
        val_loader = (
            self._get_loader(x_val, y_val, val_memory_flags, mode="train")
            if len(x_val) > 0
            else None
        )
        test_loader = self._get_loader(
            x_test, y_test, np.zeros((x_test.shape[0],)), mode="test"
        )
        ...
        if self._onehot:  # Need to add dummy zeros to match the number of targets:
            targets_memory = np.concatenate(
                (
                    targets_memory,
                    np.zeros(
                        (targets_memory.shape[0], self.increments[self._current_task])
                    ),
                ),
                axis=1,
            )

        memory_flags = np.concatenate(
            (np.zeros((x.shape[0],)), np.ones((data_memory.shape[0],)))
        )
    ...
    def get_custom_loader(
        ...
        if (not isinstance(memory, tuple) and memory is not None) or (
            isinstance(memory, tuple) and memory[0] is not None
        ):
            if len(data) > 0:
    ...
    def _get_loader(self, x, y, memory_flags, shuffle=True, mode="train", sampler=None):
        ...
        if sampler is not None and mode == "train":
            ...
            sampler = sampler(
                y, memory_flags, batch_size=self._batch_size, **self._sampler_config
            )
            batch_size = 1
        else:
            sampler = None
    ...
    def _setup_data(
        ...
        for dataset in datasets:
            ...
            logger.info(
                "Dataset {}: class ordering: {}.".format(dataset.__name__, order)
            )
...
class DummyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = self.trsf(img)
        return {"inputs": img, "targets": y, "memory_flags": memory_flag, "idx": idx}
