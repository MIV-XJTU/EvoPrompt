...
class MemoryOverSampler(BatchSampler):
    ...
    def _oversample(self, y, memory_flags):
        old_indexes = np.where(memory_flags == 1.0)[0]
        new_indexes = np.where(memory_flags == 0.0)[0]
    ...
    def __iter__(self):
        for _ in range(len(self)):
            ...
            classes = np.random.choice(
                self._classes, size=self.nb_classes, replace=False
            )
            for class_id in classes:
                class_indexes = np.random.choice(
                    ...
                    replace=bool(
                        len(self._class_to_indexes[class_id]) < self.nb_per_class
                    ),
                )
    ...
    def __iter__(self):
        ...
        while count + self._batch_size < len(self.y):
            ...
            for class_index in classes:
                ...
                class_batch_indexes = class_indexes[
                    class_counter : class_counter + self.n_samples
                ]
