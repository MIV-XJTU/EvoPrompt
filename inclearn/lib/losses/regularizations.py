...
def weights_orthogonality(weights, margin=0.0):
...
def ortho_reg(weights, config):
    ...
    x = config.get("lambda", 10.0) * (similarities - 1.0)

    return config.get("factor", 1.0) * torch.log(1 + torch.exp(x)).sum()


def global_orthogonal_regularization(
    features, targets, factor=1.0, normalize=False, cosine=False
):
    ...
    loss = torch.pow(first_moment, 2) + torch.clamp(
...
def double_margin_constrastive_regularization(
    ...
    if intra_margin is not None and K > 1:
        ...
        if regroup_intra:
            intra_losses = torch.clamp(intra_dist - intra_margin, min=0.0)
        else:
            intra_losses = torch.clamp(intra_margin - intra_dist, min=0.0)
    ...
    if inter_margin is not None and not (inter_old_vs_new and current_index == 0):
        if inter_old_vs_new:
            inter_mask = _dmr_inter_oldvsnew_mask(
                dist.shape[0], current_index, weights.device
            )
            inter_dist = dist[inter_mask]
        elif adaptative_margin and old_weights is not None:
            old_dist = _dmr_weights_distance(old_weights, square=square).to(
                weights.device
            )
            nb_old_classes = old_weights.shape[0] // K

            inter_mask_old = _dmr_inter_mask(
                old_dist.shape[0], nb_old_classes, K, weights.device
            )
            ...
            d = torch.clamp(old_inter_dist, min=0.0)
            adaptative_margins = (
            ...
            inter_margin = torch.cat(
                (
                    adaptative_margins,
                    torch.tensor(inter_margin)
                    .repeat(len(new_inter_dist))
                    .to(weights.device),
                )
            )
            assert (
                len(oldnew_inter_dist) == len(old_inter_dist) == len(adaptative_margins)
            )
        else:
        ...
        if isinstance(inter_margin, float):
            inter_losses = torch.clamp(inter_margin - inter_dist, min=0.0)
            ...
            inter_loss = torch.pow(first_moment, 2) + torch.clamp(
                second_moment - 1.0 / weights.shape[-1], min=0.0
            )
            loss += inter_loss
...
def _dmr_inter_mask(size, nb_classes, nb_clusters, device):
    ...
    for c in range(nb_classes):
        inter_mask[
            c * nb_clusters : (c + 1) * nb_clusters, (c + 1) * nb_clusters :
        ] = True
...
def _dmr_weights_distance(weights, square=True):
    ...
    dist = torch.abs(
        dist
    )  # Absolute is to handle small negatives due to numerical instability
