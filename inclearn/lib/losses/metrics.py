...
def triplet_loss(
    ...
    if harmonic_embeddings and old_features is not None:
        if harmonic_embeddings["select"] == "old":
            old_features = old_features[memory_flags.eq(1.0)]
            old_targets = targets[memory_flags.eq(1.0)]
        elif harmonic_embeddings["select"] == "all":
            old_targets = targets
        else:
            raise ValueError(
                "Unknown harmonic embeddings selection {}.".format(
                    harmonic_embeddings["select"]
                )
            )
    ...
    if triplet_selection == "all":
        ...
        triplet_losses = _select_hardest_triplets(
            distance_matrix, targets, margin=margin
        )
        loss = _aggreg_triplet_losses(triplet_losses, aggreg=aggreg)
        ...
        triplet_losses_hard = _select_hardest_triplets(
            distance_matrix, targets, margin=margin
        )
...
def _select_all_triplets(distance_matrix, triplet_mask, margin=0.2):
    ...
    if margin == "soft":
        all_triplets = torch.log(
            1 + torch.exp(anchor_positive_dist - anchor_negative_dist)
        )
    else:
    ...
    pos_triplets = valid_triplets.clamp(min=0.0)
...
def _select_hardest_triplets(distance_matrix, targets, margin=0.2):
    ...
    anchor_negative_dist = distance_matrix + max_anchor_negative_dist * (
...
def _aggreg_triplet_losses(triplet_losses, aggreg="mean"):
    if aggreg == "mean":
        ...
        nb_not_null = len(triplet_losses[triplet_losses > 0.0])
...
def _triplet(pos_distance, neg_distance, margin, aggreg="mean"):
    triplets = torch.clamp(margin + pos_distance - neg_distance, min=0.0)
...
def _triplet_facenet_sampling(features, targets, semihard=True, distance="l2squared"):
    ...
    for target in set(targets.tolist()):
        ...
        if semihard:
            ...
            an_dist = pairwise_distances[
                torch.tensor(indexes).repeat_interleave(nb_neg, 0),
            ...
            for i in range(len(ap_dist)):
                if (ap_dist[i] < an_dist[i]).any():
                    negatives.append(
                        neg_indexes[
                            (an_dist[i] == an_dist[i][ap_dist[i] < an_dist[i]].min())
                            .argmax()
                            .item()
                        ]
                    )
            ...
            negatives = np.random.choice(
                neg_indexes, size=len(_anchors), replace=False
            ).tolist()
    ...
    return (
        torch.tensor(anchor_indexes),
        torch.tensor(positive_indexes),
