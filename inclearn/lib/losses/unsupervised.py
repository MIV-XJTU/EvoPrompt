...
def unsupervised_rotations(
    inputs, memory_flags, network, apply_on="all", factor=1.0, **kwargs
):
    ...
    if apply_on == "all":
        ...
        selected_inputs = inputs[memory_flags.eq(1.0)]
    elif apply_on == "new":
        selected_inputs = inputs[memory_flags.eq(0.0)]
    else:
