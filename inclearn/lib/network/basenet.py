...
class BasicNet(nn.Module):
    def __init__(
        ...
        self._convnet_type = convnet_type
        ...
        if classifier_kwargs["type"] == "fc":
            ...
            raise ValueError(
                "Unknown classifier type {}.".format(classifier_kwargs["type"])
            )
        ...
        if self.gradcam_hook:
            ...
            logger.info(
                "Setting gradcam hook for gradients + activations of last conv."
            )
        ...
        if self.classifier_no_act:
            logger.info(
                "No ReLU will be applied on features before feeding the classifier."
            )
    ...
    def add_classes(self, n_classes, **kwargs):
        self.classifier.add_classes(n_classes, **kwargs)

    def add_imprinted_classes(self, class_indexes, inc_dataset, **kwargs):
        if hasattr(self.classifier, "add_imprinted_classes"):
            self.classifier.add_imprinted_classes(
                class_indexes, inc_dataset, self, **kwargs
            )
