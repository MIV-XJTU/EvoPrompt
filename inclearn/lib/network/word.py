...
class Word2vec(nn.Module):
    def __init__(
        ...
        self.emb, _ = get_embeddings(
            dataset, embeddings, frozen=freeze_embedding, path=data_path
        )
        if isinstance(scale_embedding, list):
            logger.info(f"Scaling semantic embedding in {scale_embedding}.")
            self.emb.weight.data = Scaler(scale_embedding).fit_transform(
    ...
    def add_linear_transform(self, bias=False):
        ...
        if bias:
            self.linear_transform.bias.data.fill_(0.0)
...
def get_embeddings(dataset, embeddings, path=None, frozen=True):
    ...
    emb.weight = torch.nn.Parameter(
        torch.FloatTensor(weights), requires_grad=not frozen
    )
...
def _get_cifar100_embeddings(embeddings_type, data_path=None):
    ...
    trimmed_weights = np.empty(
        (len(labels), gensim_model.vectors.shape[1]), dtype=np.float32
    )
...
def _get_awa2_attributes(path=None):
    ...
    attributes = np.loadtxt(
        os.path.join(
            path, "awa2/Animals_with_Attributes2/predicate-matrix-continuous.txt"
        )
    )
...
def _get_awa2_attributes_mat(path=None):
    if path is None:
        path = "/data/douillard/"
    attributes = loadmat(
        os.path.join(path, "zeroshot_split/xlsa17/data/AWA2/att_splits.mat")
    )
...
def _get_cub200_attributes(path=None):
    ...
    attributes = np.loadtxt(
        os.path.join(
            path, "CUB_200_2011/attributes/class_attribute_labels_continuous.txt"
        )
    )
    attributes = attributes / np.linalg.norm(attributes, axis=1, keepdims=True)
...
def _get_cub200_attributes_mat(path=None):
    if path is None:
        path = "/data/douillard/"
    attributes = loadmat(
        os.path.join(path, "zeroshot_split/xlsa17/data/CUB/att_splits.mat")
    )
    attributes = attributes["att"].T
...
def _get_apy_attributes_mat(path=None):
    if path is None:
        path = "/data/douillard/"
    attributes = loadmat(
        os.path.join(path, "zeroshot_split/xlsa17/data/APY/att_splits.mat")
    )
    attributes = attributes["att"].T
