...
def add_new_weights(
    network, weight_generation, current_nb_classes, task_size, inc_dataset
):
    if isinstance(weight_generation, str):
        warnings.warn(
            "Use a dict for weight_generation instead of str", DeprecationWarning
        )
    ...
    if weight_generation["type"] == "imprinted":
        ...
        network.add_imprinted_classes(
            list(range(current_nb_classes, current_nb_classes + task_size)),
            inc_dataset,
            **weight_generation,
        )
        ...
        network.add_custom_weights(
            weights, ponderate=weight_generation.get("ponderate")
        )
    else:
        raise ValueError(
            "Unknown weight generation type {}.".format(weight_generation["type"])
        )
...
def apply_kmeans(features, targets, nb_clusters, pre_normalization):
    ...
    for class_index in np.unique(targets):
        ...
        if pre_normalization:
            class_features = class_features / np.linalg.norm(
                class_features, axis=-1
            ).reshape(-1, 1)
...
def apply_knn(
    ...
    if normalize:
        features_test = features_test / np.linalg.norm(features_test, axis=-1).reshape(
            -1, 1
        )
...
def freeze_module(module: nn.Module) -> None:
    def freeze_param(param: nn.Parameter) -> None:
        if param.is_leaf:
...
def unfreeze_module(module: nn.Module) -> None:
    def unfreeze_param(param: nn.Parameter) -> None:
        if param.is_leaf:
            param.requires_grad = True
        else:
            logger.error(f"Unable to unfreeze leaf node")

    if isinstance(module, nn.Parameter):
        unfreeze_param(module)
    else:
        for param in module.parameters():
            unfreeze_param(param)
...
def expand_to_batch(x: torch.Tensor, batch_size: int, dim=0):
    ...
    return torch.unsqueeze(x, dim=dim).repeat(*shape)
