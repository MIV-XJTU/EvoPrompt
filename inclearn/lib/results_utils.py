...
def get_save_folder(model, date, label, dataset, initial_increment, increment):
    year_month, day = date[:6], date[6:8]
    week_number = math.ceil(int(day) / 7)
    label = f"{label}-{dataset}_b{initial_increment}_{increment}increment"
...
def save_results(
    results, label, model, date, run_id, seed, dataset, initial_increment, increment
):
    if "device" in results["config"]:
        del results["config"]["device"]

    folder_path = get_save_folder(
        model, date, label, dataset, initial_increment, increment
    )
...
def extract(paths, metric="avg_inc", nb_classes=None):
    ...
    for path in paths:
        ...
        if metric in ("avg_inc", "accuracy"):
            score_plot.append(
                [100 * task["accuracy"]["total"] for task in data["results"]]
            )
        elif metric == "accuracy_top5":
            score_plot.append(
                [100 * task["accuracy_top5"]["total"] for task in data["results"]]
            )
        elif metric == "old_accuracy":
            score_plot.append(
                [100 * task.get("old_accuracy", 0.0) for task in data["results"]]
            )
        elif metric == "new_accuracy":
            score_plot.append(
                [100 * task.get("new_accuracy", 0.0) for task in data["results"]]
            )
        elif metric == "unseen":
            score_plot.append(
                [
                    100 * task.get("unseen_classes_accuracy", 0.0)
            ...
            score_plot.append(
                [
                    100 * task.get("seen_classes_accuracy", 0.0)
                    for task in data["results"]
                ]
            )
        else:
        ...
        if metric in (
            ...
            for class_id in range(nb_classes):
                class_accuracies = [
                    100
                    * task["accuracy_per_class"][
                        "{:02d}-{:02d}".format(class_id, class_id)
                    ]
                    for task in data["results"]
                    if "{:02d}-{:02d}".format(class_id, class_id)
                    in task["accuracy_per_class"]
                ]
...
def plot(
    ...
    for result in results:
        ...
        try:
            bar = plt.errorbar(
                x, means, stds, label=label, marker="o", markersize=3, **kwargs
            )
        except Exception:
            print(x)
            print(means)
        ...
        if zeroshot:
            ...
            plt.plot(
                x, seen_accs[0], linestyle="dotted", color=bar.lines[0].get_color()
            )
