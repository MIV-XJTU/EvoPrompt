...
def perclass_loop(
        ...
        for batch_index, class_id in enumerate(prog_bar, start=1):
            ...
            for i in range(qt):
                ...
                if batch_size is None:
                    words = target_to_word(
                        [class_id for _ in range(len(real_features))]
                    ).to(devices[0])
                else:
                    words = target_to_word([class_id for _ in range(batch_size)]).to(
                        devices[0]
                    )
                semantic_features = word_embeddings(words)

                loss = loss_function(
                    real_features, semantic_features, class_id, words, metrics
                )
                loss.backward()
                optimizer.step()
            ...
            _print_metrics(
                metrics, prog_bar, epoch, n_epochs, batch_index, task, n_tasks
            )
    ...
    return torch.cat(list(visual_features.values())), torch.cat(
        list(visual_targets.values())
    )
        ...
        for batch_index, (x, y) in enumerate(prog_bar, start=1):
            ...
            if normalize:
                loss = loss_function(
                    ...
                    F.normalize(semantic_features, dim=1, p=2),
                )
            else:
    ...
    for epoch in range(n_epochs):
        ...
        for batch_index, (x, y) in enumerate(prog_bar, start=1):
            ...
            reconstruction = autoencoder.decoder(
                torch.cat((attributes, pred_noise), dim=-1)
            )
            ...
            dis_true_loss = F.binary_cross_entropy_with_logits(
                true_dis, torch.ones_like(true_dis).to(devices[0])
            )
            dis_fake_loss = F.binary_cross_entropy_with_logits(
                fake_dis, torch.zeros_like(fake_dis).to(devices[0])
            )
            ...
            gen_loss = F.binary_cross_entropy_with_logits(
                fake_dis, torch.ones_like(fake_dis).to(devices[0])
            )
            gen_loss.backward()
            optimizer_g.step()
