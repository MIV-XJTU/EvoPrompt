...
def kmeans(features, nb_examplars, k=5):
    ...
    indexes = []
    for c in range(k):
        c_indexes = np.random.choice(
            np.where(cluster_assignements == c)[0], size=nb_per_clusters
        )
...
def var_ratio(memory_per_class, network, loader, select="max", type=None):
    ...
    if select == "max":
        ...
        return indexes[:memory_per_class]
    raise ValueError(
        "Only possible value for <select> are [max, min], not {}.".format(select)
    )
...
def _var_ratio(sampled_probs):
    ...
    return 1.0 - hist.max(axis=1) / sampled_probs.shape[1]
