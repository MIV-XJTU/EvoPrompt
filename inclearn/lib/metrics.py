...
class MetricLogger:
    ...
    def log_task(self, ypreds, ytrue, task_size, zeroshot=False):
        ...
        self.metrics["average_accuracy"].append(
            self.metrics["accuracy"][-1]["average_accuracy"]
        )
    ...
    def last_results(self):
        results = {
            ...
            "average_accuracy": self.metrics["average_accuracy"][-1],
    ...
    def _update_accuracy_matrix(self, new_accuracy_per_class):
        for k, v in new_accuracy_per_class.items():
            if k in ["total", "average_accuracy"]:
                continue
...
def cord_metric(accuracy_matrix, only=None):
    ...
    for class_id in range(accuracy_matrix.shape[0]):
        filled_indexes = np.where(accuracy_matrix[class_id] > -1.0)[0]
...
def accuracy_per_task(ypreds, ytrue, task_size=10, topk=1):
    ...
    if task_size is not None:
        ...
        all_acc["average_accuracy"] = np.mean(acc_per_task)
    return all_acc
