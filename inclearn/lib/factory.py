...
def get_model(args):
    dict_models = {
        ...
        "zil": models.ZIL,
        "gdumb": models.GDumb,
        "evoprompt": models.EvoPrompt,
        "simplebaseline": models.SimpleBaseline,
    }
    ...
    if model not in dict_models:
        raise NotImplementedError(
            "Unknown model {}, must be among {}.".format(
                args["model"], list(dict_models.keys())
            )
        )
...
def get_data(args: dict, class_order=None):
...
def get_lr_scheduler(
    ...
    if scheduling_config is None:
        ...
        warnings.warn(
            "Use a dict not a string for scheduling config!", DeprecationWarning
        )
        scheduling_config = {"type": scheduling_config}
    elif isinstance(scheduling_config, list):
        warnings.warn(
            "Use a dict not a list for scheduling config!", DeprecationWarning
        )
    ...
    if scheduling_config["type"] == "step":
        ...
        scheduler = schedulers.CosineWithRestarts(
            ...
            factor=scheduling_config.get("factor", 1.0),
        )
        ...
        raise ValueError(
            "Unknown LR scheduling type {}.".format(scheduling_config["type"])
        )
