...
class GPSA(nn.Module):
    ...
    def forward(self, x):
        ...
        attn, attn_data = self.get_attention(x)
        v = (
            self.v(x)
    ...
    def get_attention(self, x):
        ...
        q, k = qk[0], qk[1]
        pos_score = self.rel_indices.expand(B, -1, -1, -1).type_as(x)
        unnormalized_pos_score = self.pos_proj(pos_score).permute(0, 3, 1, 2)
        unnormalized_patch_score = (q @ k.transpose(-2, -1)) * self.scale
        patch_score = unnormalized_patch_score.softmax(dim=-1)
        pos_score = unnormalized_pos_score.softmax(dim=-1)
        ...
        unnormalized_attn = (
            1.0 - torch.sigmoid(gating)
        ) * unnormalized_patch_score + torch.sigmoid(gating) * unnormalized_pos_score
        attn /= attn.sum(dim=-1).unsqueeze(-1)
        unnormalized_attn /= unnormalized_attn.sum(dim=-1).unsqueeze(-1)

        attn = self.attn_drop(attn)
        unnormalized_attn = self.attn_drop(unnormalized_attn)

        attn_data["normalized"] = attn
        attn_data["unnormalized"] = unnormalized_attn
...
class Block(nn.Module):
    ...
    def forward(self, x, mask_heads=None, task_index=1, attn_mask=None):
        if isinstance(self.attn, ClassAttention):  # Like in CaiT
            ...
            xx = self.norm1(x)
            xx, attn, v = self.attn(
                xx, mask_heads=mask_heads, nb=task_index, attn_mask=attn_mask
            )
...
class JointCA(ClassAttention):
    ...
    def forward(self, x, attn_mask=None, nb_task_tokens=1):
        B, N, C = x.shape
        q = (
            self.q(x[:, :nb_task_tokens])
            .reshape(B, nb_task_tokens, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        )
        k = (
            self.k(x)
