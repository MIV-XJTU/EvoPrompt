...
class BatchRenormalization2D(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_ch_std = torch.clamp(
            torch.std(x, dim=(0, 2, 3), keepdim=True), self.eps, 1e10
        ).to(device)
        ...
        if self.training:
            ...
            r = (
                torch.clamp(
                    batch_ch_std / self.running_avg_std, 1.0 / self.r_max, self.r_max
                )
                .to(device)
                .data.to(device)
            )
...
class CifarResNet(nn.Module):
    ...
    def _make_layer(self, Block, planes, increase_dim=False, last=False, n=None):
        ...
        if increase_dim:
            layers.append(
                Block(planes, increase_dim=True, downsampling=self._downsampling_type)
            )
            planes = 2 * planes
