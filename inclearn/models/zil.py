...
from inclearn.lib.network.classifiers import BinaryCosineClassifier, DomainClassifier
...
class ZIL(ICarl):
    ...
    def get_class_label(self, fake_class_ids):
        ...
        return torch.tensor(
            [self.inc_dataset.class_order[0][i] for i in fake_class_ids]
        )

    def get_inv_class_label(self, true_class_ids):
        """Class id in datasets ==> Class id in-code."""
        return torch.tensor(
            [self.inc_dataset.class_order[0].index(i) for i in true_class_ids]
        )
    ...
    def _before_task(self, train_loader, val_loader):
        ...
        if self._task == 0:
            ...
            utils.add_new_weights(
                self._network,
                {"type": "imprinted"}
                if self._network.classifier.classifier_type == "cosine"
                else {"type": "basic"},
                self._n_classes,
                self._task_size,
                self.inc_dataset,
    ...
    def _train_task(self, train_loader, val_loader):
        if self._cheat_pixels is not None:
            train_loader.dataset.x = np.concatenate(
                (train_loader.dataset.x, self._cheat_pixels[0])
            )
            train_loader.dataset.y = np.concatenate(
                (train_loader.dataset.y, self._cheat_pixels[1])
            )
        ...
        if self.unsupervised_config and (
            (self._task > 0 and not self.unsupervised_config.get("only_first", False))
            or self._task == 0
        ):
    ...
    def _eval_task(self, loader):
        ...
        if self.fakeclassifier_config and self._task != self._n_tasks - 1:
            ...
            if (
                hasattr(self._network.classifier, "_bias")
            ...
            if self.fakeclassifier_config["what"] == "new":
                self._network.classifier._weights.append(
                    nn.Parameter(
                        torch.randn(
                            nb_unseen_classes
                            * self._network.classifier.proxy_per_class,
                    ...
                    [
                        nn.Parameter(
                            self._preprocessing.transform(
                                self._network.classifier.weights.data
                            )
                        ),
                        nn.Parameter(
                            torch.randn(
                ...
                if self._task == 0:
                    ...
                    self._network.classifier._negative_weights, nn.Parameter
                ):
                    params.append(
                        nn.Parameter(
                            self._preprocessing.transform(
                ...
                if self._task == 0:
                    params.append(
                        nn.Parameter(
                            torch.randn(
                                nb_unseen_classes, self._network.convnet.out_dim
                            )
                        )
                    )
                elif isinstance(
                    ...
                    params.append(
                        nn.Parameter(self._network.classifier._negative_weights.data)
                    )
                else:
                    params.append(
                        nn.Parameter(self._network.classifier._negative_weights)
                    )
                self._network.classifier._weights = nn.ParameterList(params)
                ...
                nn.init.kaiming_normal_(
                    self._network.classifier._weights[0], nonlinearity="linear"
                )
                nn.init.kaiming_normal_(
                    self._network.classifier._weights[1], nonlinearity="linear"
                )
            else:
            ...
            if "training" in self.fakeclassifier_config:
                if (
                    self.fakeclassifier_config.get("only_first_task")
                    and self._task == 0
                ):
            ...
            if (
                self.fakeclassifier_config.get("postprocessing", "none")
                == "align_weights"
            ):
        ...
        if self._task != self._n_tasks - 1 and self.fakeclassifier_config:
            if self.fakeclassifier_config.get("threshold") is not None:
                ...
                ypred[maxes < threshold1, : self._n_classes] = 0.0
                ypred[maxes > threshold2, self._n_classes :] = 0.0
    ...
    def train_supervised(self, train_loader, val_loader):
        ...
        for config in self.supervised_config:
            ...
            if config.get("update_gmmn", False) and self._task not in (
                0,
                self._n_tasks - 1,
            ):
                self.train_gmmn()
                self._setup_constraints()
            if config.get("update_constraints", False) and self._task not in (
                0,
                self._n_tasks - 1,
            ):
                self._setup_constraints()
            if config.get("update_sdc", False) and self._task not in (
                0,
                self._n_tasks - 1,
            ):
                ...
                with torch.no_grad():
                    ...
                    if self._network.classifier._negative_weights is not None:
                        self._network.classifier._negative_weights = (
                            self._network.classifier._negative_weights + drift
                        )
    ...
    def train_gmmn(self):
        ...
        if to_train:
            if config.get("reinit", "always") == "always" or (
                config.get("reinit", "first") and self._task == 0
            ):
                ...
                raise NotImplementedError(
                    f"Unknown value for GMMN: {config.get('reinit')}."
                )

        optimizer = factory.get_optimizer(
            [{"params": self._word_embeddings.parameters(), "lr": config["lr"]}],
            config["optimizer"],
            config["lr"],
            config.get("weight_decay", self._weight_decay),
        )
        ...
        if config.get("preprocessing"):
            if isinstance(config["preprocessing"], list):
                ...
                self._preprocessing = Scaler(
                    (0, 1), robust=-1, normalize=True, truncate=True
                )
        ...
        self._visual_features, self._visual_targets = loops.perclass_loop(
            ...
            memory_class_ids=[]
            if self._task == 0
            else list(range(self._n_classes - self._task_size)),
    ...
    def train_autoencoder(self):
        ...
        if to_train:
            if config.get("reinit", "always") == "always" or (
                ...
                raise NotImplementedError(
                    f"Unknown value for AdvAE: {config.get('reinit')}."
                )
    ...
    def train_fake_classifier(self, config):
        ...
        if config.get("adversarial_classifier"):
            ...
            optimizer, scheduler = self.setup_training(
                ...
                additional_parameters={
                    "domain_classifier": self._domain_classifier.parameters()
                },
            )
        else:
        ...
        if isinstance(config["nb_samples"], int):
            ...
            nb_samples = int(
                torch.bincount(self._visual_targets).float().mean().cpu().item()
            )
        ...
        flags = torch.cat(
            (torch.ones(len(self._visual_features)), torch.zeros(len(fake_features)))
        )

        if (
            not isinstance(config.get("class_weights"), str)
            and config.get("class_weights") is True
        ):
            logger.info("Computing class weights.")
            ...
            self._class_weights_fake = (
                torch.tensor(class_weights).to(self._device).float()
            )
            ...
            unique_targets = np.unique(np_targets)
            class_weights = compute_class_weight("balanced", unique_targets, np_targets)
            class_weights_unseen = (
                np.ones(max_class - self._n_classes) * class_weights.mean()
            )
    ...
    def forward_unsupervised(
        ...
        loss, outputs = losses.unsupervised_rotations(
            inputs, memory_flags, training_network
        )
        ...
        if self._old_model is not None:
            ...
            if self._pod_spatial_config:
                if self._pod_spatial_config.get("scheduled_factor", False):
                    ...
                    factor = self._pod_spatial_config.get("factor", 1.0)
            ...
            if self._pod_flat_config:
                factor = self._pod_flat_config.get("factor", 1.0)
    ...
    def forward_supervised(
        ...
        **kwargs,
        ...
        if (
            self._task > 0
            and self.ghost_config
            and self.ghost_config.get("negative_weights_percent")
            and not config.get("del_neg_weights", False)
        ):
            percent = self.ghost_config["negative_weights_percent"]

            if self._task == self._n_tasks - 1:
                percent = self.ghost_config.get(
                    "negative_weights_percent_last", percent
                )
            ...
            if isinstance(percent, str) and percent == "hardest":
                ghost_targets = self.get_class_label(
                    list(
                        range(self._n_classes + self._task_size, self._total_n_classes)
                    )
                )
                ghost_targets = ghost_targets.to(self._device).long()

                ghost_similarities = self.similarity_matrix[
                ...
                if len(ghost_targets) == 0:
                    ...
                    most_similars = self.get_inv_class_label(most_similars).to(
                        self._device
                    )
                    ...
                    for real_t, ghost_t in zip(targets, most_similars):
                        ...
                        rnd_index = torch.randint(
                            low=0, high=len(sub_features), size=(1,)
                        )[0]
                    ...
                    ghost_flags = torch.cat(
                        (
                            torch.ones(len(additional_features)),
                            torch.zeros(len(targets)),
                        )
                    ).to(self._device)
        ...
        if self._task >= 2 and self.placement_config and config.get("ghost_placement"):
            ...
            self._task >= 2
            and self.adv_placement_config
            and config.get("ghost_placement")
        ):
            real_features = outputs[
                self.gmmn_config.get("features_key", "raw_features")
            ]
            ...
            if len(real_features) == 0:
                ...
                ghost_targets = (
                    torch.zeros(len(ghost_features)).float().to(self._device)
                )
        ...
        if self._nca_config:
            ...
            loss += F.cross_entropy(
                self._network.post_process(outputs["logits"]),
                targets.to(outputs["logits"].device),
            )
        ...
        if self._task > 0 and self.ucir_ranking_config:
            ...
            ranking_loss = self.ucir_ranking_config.get(
                "factor", 1.0
            ) * losses.ucir_ranking(
                r_logits,
                r_targets.to(r_logits.device),
                self._n_classes,
                self._task_size,
            )
                ...
                if len(features) == 0:
                    ...
                    if self._svm_weights is None:
                        ...
                        for sv, gamma, dual_coef, intercept in zip(
                            self._svm_weights["sv"],
                            self._svm_weights["gamma"],
                            self._svm_weights["dual_coef"],
                            self._svm_weights["intercept"],
                        ):
                    ...
                    if type_ == "anticorrelation":
                        hinges = torch.clamp(simi + 1.0, min=0.0)
                    elif type_ == "anticorrelation_neg":
                        hinges = torch.clamp(simi, min=0.0)
                    ...
                    if self.hyperplan_config.get("adamine"):
                        nb_not_null = torch.nonzero(
                            torch.clamp(hinges - 1e-6, min=0.0)
                        ).shape[0]
                    ...
                    if self.hyperplan_config.get("scheduled"):
                        hyper_loss = hyper_loss * math.sqrt(
                            self._n_classes / self._task_size
                        )
    ...
    def forward_gmmn(
        self, visual_features, semantic_features, class_id, words, metrics
    ):
        loss = mmd(
            real=visual_features, fake=semantic_features, **self.gmmn_config["mmd"]
        )
        ...
        if self.gmmn_config.get("old_mmd") and self._old_word_embeddings is not None:
            ...
            if (
                not self.gmmn_config["old_mmd"].get("apply_unseen", False)
            ...
            if _type == "mmd":
                old_loss = factor * mmd(
                    real=old_semantic_features,
                    fake=semantic_features,
                    **self.gmmn_config["mmd"],
                )
            elif _type == "kl":
                ...
                old_loss = (
                    ...
                    * (
                        ...
                        - torch.cosine_similarity(
                            semantic_features, old_semantic_features
                        )
                    ).mean()
                )
            else:
    ...
    def forward_fakeclassifier(self, logits, targets, flags, metrics):
        ...
        if self._domain_classifier is not None:
            ...
            factor = self.fakeclassifier_config["training"]["adversarial_classifier"][
                "factor"
            ]
    ...
    def _setup_constraints(self):
        ...
        self.similarity_matrix = generate_similarity_matrix(
            ...
            self.get_class_label(list(range(self._total_n_classes))),
        )
            ...
            if self.hyperplan_config.get("linear_nn"):
                ...
                self._hyperplan, self._hyperplan_bias = self._gen_hyperplan_constraints(
                    ...
                    end_normalize_bias=self.hyperplan_config.get(
                        "end_normalize_bias",
                        self.hyperplan_config.get("end_normalize", False),
                    ),
                    normalize_features=self.hyperplan_config.get(
    ...
    def _gen_linear_hyperplan_constraints(
        ...
        _, loader = self.inc_dataset.get_custom_loader(
            classes, memory=self.get_memory()
        )
            ...
            for target in torch.unique(ghost_targets):
                ...
                def func_loss(feats, t, fl, m):
                    ...
                    return F.binary_cross_entropy_with_logits(
                        feats.squeeze(1), t, weight=weight
                    )
    ...
    def _gen_hyperplan_constraints(
        ...
        if one_per_ghost:
            ...
            for class_id in np.unique(ghost_targets):
                tmp_ghost_features = ghost_features[
                    np.where(ghost_targets == class_id)[0]
                ]
                ...
                tmp_targets = np.concatenate(
                    (real_targets, np.ones(len(tmp_ghost_features)))
                )
                ...
                if kernel == "linear":
                    hyperplans.append(
                        torch.tensor(svm.coef_[0]).float().to(self._device)[None]
                    )
                    biases.append(
                        torch.tensor(svm.intercept_[0]).float().to(self._device)
                    )
                else:
            ...
            if kernel == "linear":
                ...
                self._svm_weights = {
                    "sv": [torch.tensor(svm.support_vectors_).float().to(self._device)],
                    "gamma": [
                        torch.tensor([1 / (features.shape[1] * features.var())])
                        .float()
                        .to(self._device)
                    ],
                    "dual_coef": [
        ...
        if config.get("cheat"):
            ...
            if config["cheat"] == "pixels":
                ...
                if config["cheat"] == "own":
                    ...
                    state_dict = torch.load(
                        os.path.join(
                            config["cheat"], f"net_0_task_{self._task + 1}.pth"
                        )
                    )
                ...
                for class_id in classes_to_create:
                    ...
                    if config["amount_per_class"] is not None:
                        ...
                        indexes = np.random.choice(
                            indexes, size=config["amount_per_class"]
                        )
                        f = f[indexes]
                        t = t[indexes]
                    features.append(f)
            ...
            if config.get("align_features_per_class"):
                ...
                for t in torch.unique(targets):
                    indexes = t == targets
                    new_features.append(
                        self._network.classifier.align_features(features[indexes])
                    )
        ...
        **kwargs,
        ...
        if method == "word_embeddings":
            ...
            for class_id in classes_to_create:
                ...
                weights.append(
                    self._word_embeddings(real_class_id).mean(dim=0, keepdims=True)
                )
...
def semantic_regularization(
    ...
    if metric == "cosine":
        ...
        if margin is not None:
            ...
            margins = similarity_matrix[
                targets[pair_indexes[..., 0]], targets[pair_indexes[..., 1]]
            ]

        hinges = torch.clamp(similarities - margins, min=0.0)
...
def ghost_semantic_regularization(
    ...
    **kwargs,
    ...
    if triplet:
        ...
        ap = (dists.to(features.device) * mask.to(features.device).float()).max(dim=1)[
            0
        ]
        ...
        hinges = torch.clamp(margin + ap - an, min=0.0)
        ...
        hinges = torch.clamp(similarities.view(-1) - margin, min=0.0)
        return _aggreg(hinges, aggreg, features_dim=features.shape[1])
    else:
        neg_indexes = []
        np_targets = targets.cpu().numpy()
        ...
        if metric == "cosine":
            ...
            return factor * _aggreg(
                similarities, aggreg, features_dim=features.shape[1]
            )
        elif metric == "snr":
...
def _aggreg(hinges, aggreg_method, features_dim):
    if isinstance(aggreg_method, int):
        ...
        nb_not_null = (torch.clamp(hinges - 1e-6, min=0.0) != 0.0).sum()
        if nb_not_null == 0.0:
            nb_not_null = 1.0
        ...
        return torch.pow(first_moment, 2) + torch.clamp(
            second_moment - 1.0 / features_dim, min=0.0
        )
