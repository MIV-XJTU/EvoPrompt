...
class LwF(IncrementalLearner):
    ...
    * Learning without Forgetting
    ...
    def _forward_loss(
        self, training_network, inputs, targets, memory_flags, metrics, *args, **kwargs
    ):
        ...
        loss = self._compute_loss(
            inputs, outputs, targets, onehot_targets, memory_flags, metrics
        )
    ...
    def _compute_loss(
        self, inputs, outputs, targets, onehot_targets, memory_flags, metrics
    ):
        ...
        if self._old_model is None:
            ...
            modified_old_proba = torch.pow(
                old_proba[..., : -self._task_size],
                self._distillation_config["temperature"],
            )
            ...
            distill_loss = self._distillation_config[
                "factor"
            ] * F.binary_cross_entropy_with_logits(modified_proba, modified_old_proba)
