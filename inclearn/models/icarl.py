...
from inclearn.lib.metrics import accuracy_per_task, accuracy
...
class ICarl(IncrementalLearner):
    ...
    def __init__(self, args):
        ...
        self._network = network.BasicNet(
            ...
            classifier_kwargs=args.get(
                "classifier_config", {"type": "fc", "use_bias": True}
            ),
            device=self._device,
            extract_no_act=True,
    ...
    def load_metadata(self, directory, run_id):
        if os.path.isdir(directory):
            ...
            path = os.path.join(
                os.path.dirname(directory), f"meta_{run_id}_task_{self._task}.pkl"
            )
    ...
    def _training_step(
        ...
        best_epoch, best_acc = -1, -1.0
        wait = 0

        grad, act = None, None
        if len(self._multiple_devices) > 1:
            logger.info(
                "Duplicating model on {} gpus.".format(len(self._multiple_devices))
            )
            ...
            if self._network.gradcam_hook:
                ...
                training_network.module.convnet.last_conv.register_backward_hook(
                    back_hook
                )
                training_network.module.convnet.last_conv.register_forward_hook(
                    for_hook
                )
        else:
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            if (
                epoch == nb_epochs - 1
                and record_bn
                and len(self._multiple_devices) == 1
                and hasattr(training_network, "convnet")
                and hasattr(training_network.convnet, "record_mode")
            ):
            ...
            self._num_training_batches = len(prog_bar)
            ...
            self.on_train_epoch_end(prog_bar2, training_network)

            if self._scheduler:
                self._scheduler.step(epoch)

            if (
                self._eval_every_x_epochs
                and epoch != 0
                and epoch % self._eval_every_x_epochs == 0
            ):
                self._network.eval()
                if (
                    not hasattr(self, "_build_examplars_every_x_epochs")
                    or self._build_examplars_every_x_epochs
                ):
                    (
                        self._data_memory,
                        self._targets_memory,
                        self._herding_indexes,
                        self._class_means,
                    ) = self.build_examplars(self.inc_dataset, self._herding_indexes)
                ypred, ytrue = self._eval_task(val_loader)
                acc = accuracy(ypred, ytrue)
                task_size = self._increment
                if self.domain_incremental:
                    task_size = self._n_classes
                acc_per_task = accuracy_per_task(
                    ypred, ytrue, task_size=task_size, topk=1
                )
                print(
                    f"T{self._task}/{self._n_tasks-1}, E{epoch}/{nb_epochs-1} => Val accuracy: {acc}"
                )
                print(
                    f"T{self._task}/{self._n_tasks-1}, E{epoch}/{nb_epochs-1} => Val acc_per_task: {acc_per_task}"
                )
                wandb.log({f"task_{self._task}-acc": acc, "epoch": epoch})
                wandb.log({"acc_over_steps": acc})
                if (
                    hasattr(self, "_log_past_task_accuracy")
                    and self._log_past_task_accuracy
                    and self._task > 0
                ):
                    self.log_past_task_accuracy(acc_per_task)
        ...
        if self._eval_every_x_epochs:
            logger.info(
                "Best accuracy reached at epoch {} with {}%.".format(
                    best_epoch, best_acc
                )
            )

        if (
            len(self._multiple_devices) == 1
            and hasattr(training_network, "convnet")
            and hasattr(training_network.convnet, "record_mode")
        ):
    ...
    def log_past_task_accuracy(self, acc_per_task: Dict[str, float]):
        acc = copy.deepcopy(acc_per_task)
        if "total" in acc:
            acc.pop("total")
        if "average_accuracy" in acc:
            acc.pop("average_accuracy")

        acc_keys = list(acc.keys())
        acc_keys.sort()
        sorted_acc = {i: acc[i] for i in acc_keys}
        acc_all = list()
        for task, k in enumerate(sorted_acc.keys()):
            if task < len(acc_keys) - 1:
                acc_all.append(sorted_acc[k])

        past_task_acc = np.mean(acc_all)
        wandb.log({f"past_average_acc": past_task_acc})
    ...
    def _print_metrics(self, prog_bar, epoch, nb_epochs, nb_batches):
        ...
        for metric_name, metric_value in self._metrics.items():
            wandb.log(
                {metric_name: metric_value / nb_batches}, step=self._global_batch_idx
            )

        prog_bar.set_description(
            "T{}/{}, E{}/{} => {}".format(
                self._task, self._n_tasks - 1, epoch, nb_epochs - 1, pretty_metrics
            )
        )
    ...
    def _forward_loss(
        ...
        **kwargs,
        ...
        loss = self._compute_loss(
            inputs, outputs, targets, onehot_targets, memory_flags
        )
    ...
    def _after_task_intensive(self, inc_dataset):
        ...
        (
            ...
            self._herding_indexes,
            self._class_means,
        ) = self.build_examplars(inc_dataset, self._herding_indexes)
    ...
    def _compute_confusion_matrix(self):
        use_validation = self._validation_percent > 0.0
    ...
    def _eval_task(self, data_loader):
        ypreds, ytrue = self.compute_accuracy(
            self._network, data_loader, self._class_means
        )
    ...
    def build_examplars(
        ...
        if hasattr(self, "on_build_examplars"):
            self.on_build_examplars(inc_dataset)

        data_memory, targets_memory = [], []
        class_means = np.zeros((self._n_classes, self._network.features_dim))

        for class_idx in tqdm(range(self._n_classes), "build examplars"):
            ...
            features_flipped, _ = utils.extract_features(
                ...
                inc_dataset.get_custom_loader(
                    class_idx, mode="flip", data_source=data_source
                )[1],
            )
                ...
                if self._herding_selection["type"] == "icarl":
                    ...
                    selected_indexes = herding.confusion(
                        *self._last_results,
                        memory_per_class,
                        class_id=class_idx,
                        minimize_confusion=self._herding_selection[
                            "minimize_confusion"
                        ],
                    )
