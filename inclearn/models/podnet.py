    ...
    def _train_task(self, train_loader, val_loader):
        ...
        if self._finetuning_config and self._task != 0:
            ...
            if self._finetuning_config["scaling"]:
                logger.info(
                    "Custom fine-tuning scaling of {}.".format(
    ...
    def _before_task(self, train_loader, val_loader):
        ...
        if self._groupwise_factors and isinstance(self._groupwise_factors, dict):
            ...
            for (
                group_name,
                group_params,
            ) in self._network.get_group_parameters().items():
            ...
            params = [
                {"params": self._network.convnet.parameters(), "lr": self._lr},
                {"params": self._network.classifier.new_weights, "lr": self._lr},
            ]
        else:
        ...
        if self._class_weights_config:
            self._class_weights = torch.tensor(
                data.get_class_weights(
                    train_loader.dataset, **self._class_weights_config
                )
            ).to(self._device)
        else:
        ...
        if self._old_model is not None:
            ...
            if self._gradcam_distil:
                ...
                if self._gradcam_distil.get("scheduled_factor", False):
                    ...
                    factor = self._gradcam_distil.get("factor", 1.0)
