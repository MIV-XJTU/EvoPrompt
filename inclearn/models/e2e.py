...
class End2End(IncrementalLearner):
    ...
    def _train_task(self, train_loader, val_loader):
        ...
        if self._task == 0:
            ...
            optimizer = factory.get_optimizer(
                self._network.parameters(), self._opt_name, 0.1, 0.0001
            )
            scheduler = torch.optim.lr_scheduler.MultiStepLR(
                optimizer, [50, 60], gamma=0.1
            )
        ...
        optimizer = factory.get_optimizer(
            self._network.parameters(), self._opt_name, 0.01, 0.0001
        )
    ...
    def _train(self, train_loader, val_loader, n_epochs, optimizer, scheduler):
        ...
        for epoch in range(n_epochs):
            ...
            _clf_loss, _distil_loss = 0.0, 0.0
            c = 0
            ...
            for i, (inputs, targets, _) in prog_bar:
                ...
                clf_loss, distil_loss = self._compute_loss(inputs, logits, targets)

                if not utils._check_loss(clf_loss) or not utils._check_loss(
                    distil_loss
                ):
                    import pdb
    ...
    def _compute_loss(self, inputs, logits, targets):
        ...
        if self._task == 0:
            ...
            for i in range(last_index):
                ...
                distil_loss += F.binary_cross_entropy(task_prob_new, task_prob_old)
    ...
    def _reduce_examplars(self):
        ...
        for class_idx in self._data_memory.keys():
            self._data_memory[class_idx] = self._data_memory[class_idx][
                : self._memory_per_class
            ]
            self._targets_memory[class_idx] = self._targets_memory[class_idx][
                : self._memory_per_class
            ]
